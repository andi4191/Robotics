...
class BayesianFil():
	...
	def collect_non_zero_points(self,grid):
		...
		print "####NUM: ",np.count_nonzero(self.bel)
		return points
		...
		if(topic=='Observations'):
			...
			for i in range(0,len(points)):
				...
				if(d<min_d):
					...
					min_d_ang=999
					tm=self.bel[sol_point.x][sol_point.y][:].tolist()
					sol_point.z=tm.index(np.max(tm))
					...
					for th in range(0,self.odim):
						...
						if(abs(th_delta)<min_d_ang):
							...
							sol_point.z=th
					'''
			print "closest_non_zero_point",sol_point.x,sol_point.y,sol_point.z,min_d
			for i in range(0,len(points)):
