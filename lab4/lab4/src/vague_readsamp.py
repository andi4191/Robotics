...
class BayesianFil():
	...
	def distribute_new_orien(self,new_grid,move_blocks,point,val):
		...
		if(val!=0):
			...
			#new_grid[point.x][point.y][(new_z-2)%self.odim]+=val*c3
			#new_grid[point.x][point.y][(new_z+2)%self.odim]+=val*c3
	...
	def distr_new_pos(self,grid,new_pos):
	...
	def interpolate_prob(self,pos,grid):
		
		dim=3
		sig=1
		kern3=self.gkern3(dim,sig)
		new_grid=np.zeros((self.tdim,self.tdim,self.odim),dtype='float')
		tmp_mat=self.copy_mat3(new_grid,kern3,pos,3)
		new_grid=np.add(new_grid,grid)
		return new_grid
	...
	def handlemsg(self,msg,topic):
		...
		if(topic=='Observations'):
			...
			#new_grid=np.zeros((self.tdim,self.tdim,self.odim),dtype='float')
			new_grid=self.distr_new_pos(self.bel,new_pos)
			
			
			###		START        ########
			'''
			tmp_grid=self.interpolate_prob(new_pos,self.bel)
			self.bel=np.copy(tmp_grid)
			self.update_pos()
			sol_point=Point()
			sol_point.x=self.px
			sol_point.y=self.py
			...
			for i in range(0,self.tdim):
				for j in range(0,self.tdim):
					for k in range(0,self.odim):
						if(i==self.px and j==self.py and k==self.pz):
							continue
						else:
			...
			grid[sol_point.x][sol_point.y][sol_point.z]=1
			mat=self.gkern3(5,2)
			tmp_mat=self.copy_mat3(grid,mat,sol_point,3)
			self.bel=np.copy(tmp_mat)
			'''
				
			
			#upd_points.append(sol_point)
			print "num of non_zero points",np.count_nonzero(self.bel)
			...
			for i in range(0,len(points)):
				p=Point()
				p.x=points[i].x+int(dist*np.cos(ang1))
				p.y=points[i].y+int(dist*np.sin(ang1))
				
				#Current angle should be considered as well
			
				tmp=self.bel[points[i].x][points[i].y][:].tolist()
				idx=tmp.index(np.max(tmp))
			
				indx=self.get_coordinate_for_radian(ang1)
				p.z=indx
				temp_pos.append(p)
			fin_points=[]
			for i in range(0,len(temp_pos)):
				...
				indx=self.get_coordinate_for_radian(ang2)
				pt.z=indx
				fin_points.append(pt)
